"use strict";function removeComment(e){$.ajax({type:"POST",url:"/delete-comment",data:{commentId:window.event.target.id,pathname:window.location.pathname},success:function(e,o,n){window.location.pathname=e.redirect}})}var commentSkipNum=0;window.location.pathname.indexOf("projects/")>=0&&(console.log(window.location.pathname),document.getElementById("loadMoreComments").addEventListener("click",function(e){e.preventDefault(),commentSkipNum++,$.ajax({type:"GET",data:{commentSkipNum:commentSkipNum},url:window.location.pathname,success:function(e){e?(console.log("*** DATA \n",e),$("#commentList").append(e)):(console.log("NO DATA"),$("#loadMoreComments").removeClass("btn-info").addClass("disabled").text("No More"))},error:function(e){console.log("Error \n",e)}})},!1));
...
function stripeResponseHandler(e,r){var t=$("#payment-form");if(r.error)t.find(".payment-errors").text(r.error.message),t.find(".payment-errors").addClass("alert alert-danger stripe-error"),t.find("button").prop("disabled",!1);else{var n=r.id;t.append($('<input type="hidden" name="stripeToken" />').val(n)),t.get(0).submit()}}Stripe.setPublishableKey("pk_test_Xn5nWxjEXpqZ4SJQQs1cyB2K"),jQuery(function(e){e("#payment-form").submit(function(r){var t=e(this);return t.find("button").prop("disabled",!0),Stripe.card.createToken(t,stripeResponseHandler),!1})});